...
function custom_breadcrumb_li($link = '', $title = '', $position = 1)
{
    ...
    if ($position > 1) {
        $html .= icon("icon-bleu", 13, 13);
    }
    if ($link) :
        $html .= '<a href="' . $link . '" itemprop="item" aria-current="location"><span itemprop="name">' . $title . '</span></a>';
    else :
        $html .= '<span itemprop="item" aria-current="location">' . $title . '</span>';
    endif;
